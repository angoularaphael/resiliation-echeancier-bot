 ...
 * (sauf 2 impayés consécutifs : résil tout de suite, peu importe le motif).
 */
...
function hasConsecutiveTimestamps(timestamps = []) {
  const uniq = [...new Set((timestamps || []).map(Number).filter((n) => Number.isFinite(n) && n > 0))].sort(
    (a, b) => a - b
  );
  for (let i = 1; i < uniq.length; i += 1) {
...
/** Deux échéances impayées d’affilée, peu importe le motif SEPA. */
function isTwoConsecutiveUnpaid(classified = {}) {
  const unpaidCount = Number(classified.unpaidCount || classified.unpaid_count || 0);
  if (unpaidCount < 2) return false;
  if (hasConsecutiveTimestamps(classified.timestamps)) return true;
  if (hasConsecutiveMonthKeys(classified.months)) return true;
  return classified.hasPrevious === true && classified.hasCurrent === true;
}

/**
 * 2 impayés consécutifs → résilier (peu importe le motif).
 * MD06/MS02/MS03/AC04/JSON → immédiat ; fiche sans contact → immédiat ;
 * 1 impayé AM04/MD01 → relances jusqu’à 10e tentative.
 */
function shouldCancel(memberState, classified, { force = false } = {}) {
  ...
  if ((classified.policy?.cancelAt ?? 2) === 1 && Number(classified.unpaidCount || 0) >= 1) return true;
  if (isTwoConsecutiveUnpaid(classified)) return true;
...
function cancelWhy(classified = {}) {
  ...
  if (isTwoConsecutiveUnpaid(classified)) return 'two_consecutive_unpaid';
  const cancelAt = classified.policy?.cancelAt ?? INSUFFICIENT_FUNDS_CANCEL_AT;
  const reasons = classified.sepaReasons || [];
  if (cancelAt === 1 && Number(classified.unpaidCount || 0) >= 1) {
