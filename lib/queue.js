...
function listPending() {
  ...
  return fs
    ...
    .sort((a, b) => {
      // Vérifs identité (changement abo) avant les autres — UX front
      const pa = String(a.action || '') === 'verify_identity' ? 0 : 1;
      const pb = String(b.action || '') === 'verify_identity' ? 0 : 1;
...
function isEcheancierJob(data = {}) {
  const action = String(data.action || data.raw?.action || '').toLowerCase();
  const id = String(data.order_id || data.job_id || '');
  return action === 'echeancier' || /^ECHEANCIER-/i.test(id);
}

/** Retire les scans échéancier encore en file (lancement : laisser la place à Aventure). */
function purgeEcheancierJobs() {
  initQueue();
  let count = 0;
  for (const f of fs.readdirSync(QUEUE_DIR)) {
    if (!f.endsWith('.json') || f === 'processed-orders.json') continue;
    ...
    if (!isEcheancierJob(data)) continue;
    fs.unlinkSync(full);
    count += 1;
  }
  return count;
